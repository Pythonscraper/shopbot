...
async def checkout(message, state):
    ...
    async with state.proxy() as data:
        ...
        for title, price, count_in_cart in data['products'].values():
            ...
            answer += f'<b>{title}</b> * {count_in_cart}шт. = {tp}сум\n'
            total_price += tp

    await message.answer(f'{answer}\nОбщая сумма заказа: {total_price}сум.',
...
async def process_name(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if 'address' in data.keys():
            ...
            await CheckoutState.next()
            await message.answer('📱Укажите свой номер телефона чтобы мы с вами связались.',
                                 reply_markup=back_markup())
...
async def process_confirm(message: Message, state: FSMContext):
    ...
    if enough_money:
        ...
        async with state.proxy() as data:
            ...
            await message.answer('Спасибо! Ваш заказ принять 🚀\nИмя: <b>' + data['name'] + '</b>\nНомер телефона: <b>' + data['address'] + '</b>',
                                 reply_markup=markup)
            await message.answer('Для раздела меню нажмите -> /menu')
    else:
